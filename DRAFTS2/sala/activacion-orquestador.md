...
| Qué | Ruta | Para qué |
|-----|------|----------|
| Protocolo de sala | `DRAFTS2/sala/README.md` | Reglas, convenciones, qué puede y no puede un agente |
| Protocolo transversal | `mod/instructions/sala-protocolo.instructions.md` | Disco > chat, checkpoints, entrega, cierre atómico |
| Big picture del mod | `mod/instructions/onboarding-map.instructions.md` | Directorios, cadena de agentes, protocolo |
...
| Carpetas de agentes | `DRAFTS2/sala/agente-*/` | Trabajo temporal en curso |
| Estado de cada agente | `DRAFTS2/sala/agente-*/estado.md` | Canal de comunicación con cada agente |

### 2.3 Dossiers (read-only, referencia)

| Dossier | PLAN |
|---------|------|
| gap-corto-universo | `DRAFTS2/gap-corto-universo/PLAN_GAP_CORTO_UNIVERSO.md` |
| extraccion-sala-sdk | `DRAFTS2/extraccion-sala-sdk/PLAN_EXTRACCION_SALA_SDK.md` |
...
Lee `DRAFTS2/sala/tablero.md`. Cuenta cuántas tareas hay en cada estado:
...
- ¿Hay carpetas de agente sin tarea? (si `estado.md` dice `handshake-pendiente`, es válido)
- ¿Hay entregas pendientes?

### 3.2 Canal de agentes (estado.md)

Lista las carpetas `DRAFTS2/sala/agente-*/`. Para cada una, lee `estado.md` y reporta:
- Alias y modelo
...
## Nota sobre rama feat/sala-sdk

Las tasks ES-02..ES-05 del track ES trabajan sobre la rama `feat/sala-sdk`. Los agentes deben producir sus artefactos candidatos en su carpeta temporal (`DRAFTS2/sala/agente-{alias}/`). Aleph se encarga de copiarlos a `.github/` en la rama correcta al cerrar.
---
name: sala-aleph
description: "Activa al orquestador de la sala de coordinación. Levanta contexto completo: protocolo, estado de dossiers, agentes, salud del tablero. Usa 'eres Aleph' para invocar."
---
